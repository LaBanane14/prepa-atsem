...
/**
 * Calcule le score d'une question selon la famille de barème.
 * @param {number} familyId — 1, 2, 3 ou 4
 * @param {string[]} userAnswers — ['a', 'c'] (réponses cochées par le candidat)
 * @param {string[]} correctAnswers — ['a', 'b', 'c'] (bonnes réponses)
 * @returns {{ points: number, status: 'perfect'|'partial'|'error'|'empty' }}
 */
export function scoreQuestion(familyId, userAnswers, correctAnswers) {
  ...
  // Abstention
  if (userSet.size === 0) {
    if (familyId === 4) return { points: -0.5, status: 'empty' }
  ...
  // Partiel sans erreur (a coché une partie des bonnes réponses sans aucune mauvaise)
  const isPartial = !hasWrong && !allCorrectChecked && userSet.size > 0
  if (isPartial) {
    if (familyId === 1) return { points: 0, status: 'partial' } // tout ou rien
    return { points: 0.5, status: 'partial' } // F2, F3, F4 : partiel récompensé
  }

  // Au moins une erreur
  if (familyId === 1 || familyId === 2) return { points: 0, status: 'error' }
  return { points: -0.5, status: 'error' } // F3, F4 : pénalité
}
