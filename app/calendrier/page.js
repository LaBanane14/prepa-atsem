...
export default function CalendrierPage() {
  ...
  return (
    <div className="min-h-screen text-slate-900" style={{ background: 'linear-gradient(180deg, #ffffff 0%, #f3f0ff 15%, #ede9fe 30%, #f5f3ff 50%, #faf5ff 65%, #fdf4ff 80%, #fce7f3 100%)' }}>
      ...
      {/* ─── DATES NATIONALES (au-dessus de la carte) ─── */}
      <section className="pb-8">
        <div className="max-w-5xl mx-auto px-4">
          <div className="bg-white rounded-3xl border border-slate-200 p-5 sm:p-6">
            <h2 className="text-sm font-bold text-slate-900 uppercase tracking-wider mb-4 flex items-center gap-2">
              <svg className="w-4 h-4 text-purple-600" fill="none" stroke="currentColor" strokeWidth="2" viewBox="0 0 24 24"><rect x="3" y="4" width="18" height="18" rx="2"/><line x1="16" y1="2" x2="16" y2="6"/><line x1="8" y1="2" x2="8" y2="6"/><line x1="3" y1="10" x2="21" y2="10"/></svg>
            ...
            <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-5 gap-3 sm:gap-4">
              {[
                ...
                <div key={i} className="bg-slate-50 rounded-xl p-3">
                  <p className="text-[10px] font-black text-purple-600 uppercase tracking-widest mb-1">{d.label}</p>
                  <p className="text-sm font-bold text-slate-900 leading-tight">{d.value}</p>
                </div>
              ))}
            </div>
          </div>
