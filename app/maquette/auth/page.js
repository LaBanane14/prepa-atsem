...
// Les entraînements et thèmes affichés dans la pile (repris du hero de l'accueil maquette)
const MODULES = [
    ...
    <section className={`relative lg:grid lg:grid-cols-2 min-h-screen overflow-hidden ${sombre ? 'bg-[#0d0d0d]' : ''}`}>
      ...
      <div className="relative flex items-center justify-center px-6 pt-[110px] pb-14 min-h-screen">
        ...
        <div className={`max-w-[420px] w-full ${sliding}`} style={{marginTop: '3vh'}}>
          ...
          <p className={`mt-8 text-center text-xs font-medium leading-relaxed ${sombre ? 'text-white/40' : 'text-black/35'}`}>En continuant sur Prépa ATSEM, vous acceptez nos <a href="/maquette/cgu" className={`underline transition ${sombre ? 'hover:text-white/75' : 'hover:text-black/60'}`}>Conditions Générales d&apos;Utilisation</a>.</p>
        </div>
        )}
      </div>
